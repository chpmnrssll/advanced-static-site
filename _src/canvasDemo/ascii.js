...
function getImageDataAverage(imageData) {
...
export default class Ascii {
  ...
  convertImage() {
    ...
    for (let y = 0; y < this.canvas.height; y += this.fontHeight) {
      for (let x = 0; x < this.canvas.width; x += this.fontWidth) {
        ...
        const imageData = this.context2D.getImageData(x, y, this.fontWidth, this.fontHeight).data;

        // find matching character from imageData average (lightness)
        const character = this.getMatchingCharacter(imageData);
  ...
  generateCharacters(characterString) {
    ...
    characters.forEach((character) => {
      const average = this.getCharacterAverage(character);
      if (average) {
        characters.push({
          character,
          average,
  ...
  getCharacterAverage(character) {
    ...
    const imageData = this.context2D.getImageData(0, 0, this.canvas.width, this.canvas.height).data;
    const average = getImageDataAverage(imageData);
  ...
  getMatchingCharacter(imageData) {
    ...
    this.characterList.forEach((character) => {
      if (character.average) {
        const difference = Math.abs(average.lightness - character.average.lightness);
        if (difference < min) {
          min = difference;
          best = character;
        }
