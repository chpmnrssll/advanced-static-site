  ...
  update() {
    ...
    for (let y = this.canvas.height - 2; y > 0; y--) {
      for (let x = 0; x < this.canvas.width; x++) {
        ...
        const r1 = this.buffer.data[index++];
        const g1 = this.buffer.data[index++];
        const b1 = this.buffer.data[index++];

        // X+1: Right pixel
        index = (this.yIndex[y] + x + 1) << 2;
        const r2 = this.buffer.data[index++];
        const g2 = this.buffer.data[index++];
        const b2 = this.buffer.data[index++];

        // Y+1: Bottom pixel
        index = (this.yIndex[y + 1] + x) << 2;
        const r3 = this.buffer.data[index++];
        const g3 = this.buffer.data[index++];
        const b3 = this.buffer.data[index++];

        const turbulence = this.turbulence[
          (this.yIndex[y] + x + this.turbulenceIndex) % this.turbulence.length
        ];
