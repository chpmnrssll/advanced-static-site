...
export default class NormalMap {
  constructor(canvas, width = 256, height = 256, image, normalMap) {
    this.canvas = canvas;
    ...
    this.texture.normalMap.src = normalMap;
  ...
  loadHandler() {
    if (++this.imagesLoaded === 2) {
      ...
      const textureImageData = this.context2D.getImageData(
      ...
      const normalMapImageData = this.context2D.getImageData(
        0, 0, this.canvas.width, this.canvas.height,
      );
      ...
      for (let j = 0; j < this.texture.normals.length; j += 4) {
        const x = (this.texture.normals[j] / 255) * 2 - 1;
        const y = (this.texture.normals[j + 1] / 255) * 2 - 1;
        const z = (this.texture.normals[j + 2] / 255) * 2 - 1;
  ...
  update() {
    ...
    const { diffuse, normals } = this.texture;
    const output = this.buffer.data;

    for (let i = 0; i < output.length; i += 4) {
      // get the pixel color from the texture. This is the diffuse color.
      const diffuseR = diffuse[i];
      const diffuseG = diffuse[i + 1];
      const diffuseB = diffuse[i + 2];
      ...
      // const j = i / 4
      const tmp = i >> 2;
      const fragmentX = tmp % this.canvas.width;
      const fragmentY = (tmp / this.canvas.width) >> 0; // bitwise Math.floor
      ...
      const lightDist = length(lightDirX, lightDirY, lightDirZ);
      ...
      // dot product of light direction and surface normal (the direction that
      // pixel of surface is 'pointing') negative values indicate that the pixel
      // is pointing away from the light, so it will not be illuminated (clamped
      // to 0) positive values indicate that teh pixel is pointing towards the
      // light, so it be illuminated based on the angle (0 to 1)
      const dp = dot(lightDirX, lightDirY, lightDirZ, normalX, normalY, normalZ);
      const reflection = dp < 0 ? 0 : dp;

      // this will reduce the effect of the point light based on distance
      const attenuation = this.pointLight.attenuateDistance(lightDist);

      // this is the effect of the point light on this pixel
      const rXa = reflection * attenuation;
      const pointR = this.pointLight.r * this.pointLight.a * rXa;
      const pointG = this.pointLight.g * this.pointLight.a * rXa;
      const pointB = this.pointLight.b * this.pointLight.a * rXa;

      // this is the effect of the ambient light on this pixel (same for all pixels)
      const ambientR = this.ambientLight.r * this.ambientLight.a;
      const ambientG = this.ambientLight.g * this.ambientLight.a;
      const ambientB = this.ambientLight.b * this.ambientLight.a;
