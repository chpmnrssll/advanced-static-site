export default class Plasma {
  ...
  update() {
    ...
    for (let y = 0; y < this.canvas.height; y++) {
      const yc = 128
        + ((this.sinc[(y << 1) + (this.tick >> 1)]
          + this.sinc[y + (this.tick << 1)]
          + (this.sinc[(y >> 1) + this.tick] << 1)) >> 6);
      for (let x = 0; x < this.canvas.width; x++) {
        const xc = 128
          + (((this.sinc[x + (this.tick << 1)] << 1)
            + this.sinc[(x << 1) + (this.tick >> 1)]
            + (this.sinc[x + this.tick] << 1)) >> 6);

        let index = (this.yIndex[y] + x) << 2;
        const color = Math.abs(((yc * xc) >> 5) % 255);
